...
(function($) {
    ...
    var swipeListener = (function() {
        var swipeListener = function(elID, opts) {
            ...
            this.el.addEventListener('touchstart', function(e) { that.touchStart(e); }, false);
            this.el.addEventListener('touchmove', function(e) { that.touchMove(e); }, false);
            this.el.addEventListener('touchend', function(e) { that.touchEnd(e); }, false);
            for (j in opts) {
                this[j] = opts[j];
            }
        };
...
//                    event.stopPropagation();    //阻止冒泡事件 by niehongliang
                }
            },
...
//                event.stopPropagation();    //阻止冒泡事件 by niehongliang
                ...
                console.log('movingX:' + this.movingX + ' movingY:' + this.movingY);
                if (Math.abs(this.movingX) > Math.abs(this.movingY))
                {
                    event.preventDefault();
                    event.stopPropagation();    //阻止冒泡事件来禁止滚动 by niehongliang
                }
            },
